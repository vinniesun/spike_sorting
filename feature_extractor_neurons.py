...
class DTLIF(nn.Module):
    def __init__(
        ...
        # Check if beta is a torch.Tensor, if not convert it to one
        if not isinstance(beta, torch.Tensor):
            beta = torch.as_tensor(beta, dtype=torch.float32)
        # check if beta is learnable or not
        if learn_beta:
            self.beta = nn.Parameter(beta)
        else:
            self.register_buffer("beta", beta)

        # check if pos_threshold and neg_threshold are torch.Tensors, 
        # if not convert them to ones
        ...
        # check if pos_threshold and neg_threshold are learnable or not
        if learn_threshold:
