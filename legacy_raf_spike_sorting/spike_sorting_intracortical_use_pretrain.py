...
def train(
    ...
    for epoch in tqdm(range(NUM_EPOCHS)):
        ...
        for data, label in train_loader:
            ...
            if USE_L1_NORM:
                loss = loss_fn(spk_out, label) + 0.001 * torch.norm(net.fc1.weight, p=1) + 0.001 * torch.norm(net.fc1.bias, p=1)
            else:
...
if __name__ == "__main__":
    ...
    NUM_EPOCHS= 20 # 20 should be enough to finetune
    ...
    with open(TRAINING_LOG_NAME, "a") as f:
        f.write(f"Using l1 norm for loss function: {USE_L1_NORM}\n")
        f.write(f"Using pretrained weights for refinement\nSeed Number: {SEED}\n\n")
