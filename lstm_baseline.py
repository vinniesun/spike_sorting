...
if __name__ == "__main__":
    ...
    thresholds = np.array([0.2]) # to do 0.6
    with open(TRAINING_LOG_NAME, "a") as f:
        f.write(f"Current Threshold: {thresholds}\n\n")
    window_size = 70
    ...
    with open(TRAINING_LOG_NAME, "a") as f:
        f.write(f"Current Training Setting: use_dm - {use_dm}, use_reconstructed - {use_reconstructed}, reconstruct_dm - {reconstruct_dm}\n\n")
    ...
    for difficulty in ["Easy1", "Easy2", "Difficult1", "Difficult2"]:
        for gt_noise_level in ["005", "01", "015", "02"]:
            ...
            with open(TRAINING_LOG_NAME, "a") as f:
                f.write(f"Current File: {filename}\n\n")
            ...
            # t1, t2, new_spk_train = calc_t1_t2_interval_filter(train_spk_train[2789], sampling_interval, spike_pulse_1ms_idx_length*2, spk_train_id=2789)
            # plot_single_spike_signal_with_dm(train_signal[2789], train_spk_train[2789], train_label[2789], new_spk_train)
            ...
            test(net, test_loader, acc_fn, final_test=True)
        print("")
